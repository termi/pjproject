...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
    ...
    if (call_info.state == PJSIP_INV_STATE_DISCONNECTED) {
        ...
        // todo: replace 'Call %d disconnected' reading in frontend to reading 'Call %d stream %d destroyed, dumping media stats..' from on_stream_destroyed()
        PJ_LOG(5,(THIS_FILE,
              "Call %d disconnected, dumping media stats..",
              call_id));
        // ADDED BY TERMI END
                                ...
                                unsigned stream_idx)
{
    ...
    /* Now pjsua_media_channel_deinit() automatically log the call dump. */
    if (0) {
        // ADDED BY TERMI: todo: send 'stream destroyed' event
        PJ_LOG(5,(THIS_FILE, 
