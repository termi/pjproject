...
static void ui_input_url_parse_buf(char *buf, input_result *result)
{
    int len = strlen(buf);
...
static void ui_call_transfer(pj_bool_t no_refersub, char menuin[])// ADDED BY TERMI (`, char menuin[]`)
{
    if (current_call == -1) {
        PJ_LOG(3,(THIS_FILE, "No current call"));
    } else {
...
static void ui_call_transfer_replaces(pj_bool_t no_refersub, char menuin[])// ADDED BY TERMI (`, char menuin[]`)
{
    if (current_call == -1) {
        PJ_LOG(3,(THIS_FILE, "No current call"));
    } else {
...
static void ui_send_dtmf_2833(char menuin[])// ADDED BY TERMI (`char menuin[]`)
{
    if (current_call == -1) {
        PJ_LOG(3,(THIS_FILE, "No current call"));
        ...
        if (strlen(menuin) > 2 && menuin[1]=='-') {
            // reading `buf` (DTMF strings to send) from `menuin`
...
static void ui_send_dtmf_info(char menuin[])// ADDED BY TERMI (`char menuin[]`)
{
    if (current_call == -1) {
        PJ_LOG(3,(THIS_FILE, "No current call"));
    } else {
 ...
 */
static void ui_change_online_status(char menuin1[])// ADDED BY TERMI (`char menuin1[]`)
{
    char menuin[32];
...
static void ui_adjust_volume(char menuin[])// ADDED BY TERMI (`char menuin[]`)
{
    char buf[128];
    char text[128];
...
void legacy_main(void)
{
  ...
  (!defined(PJ_WIN32_WINCE) || PJ_WIN32_WINCE==0)
        ...
        case 'm':
            /* Make call! : */
            ui_make_new_call(menuin);// ADDED BY TERMI (`menuin`)
            break;
        ...
        case 'a':
            ui_answer_call(menuin);// ADDED BY TERMI (`menuin`)
            break;
        ...
        case 'x':
             ...
             * Transfer call.
             */
            ui_call_transfer(app_config.no_refersub, menuin);// ADDED BY TERMI (`, menuin`)
            break;
        ...
        case 'X':
             ...
             */
            ui_call_transfer_replaces(app_config.no_refersub, menuin);// ADDED BY TERMI (`, menuin`)
            break;
        ...
        case '#':
             ...
             * Send DTMF strings.
             */
            ui_send_dtmf_2833(menuin);// ADDED BY TERMI (`menuin`)
            break;

        case '*':
            /* Send DTMF with INFO */
            ui_send_dtmf_info(menuin);// ADDED BY TERMI (`menuin`)
            break;
        ...
        case 'T':
            ui_change_online_status(menuin);// ADDED BY TERMI (`menuin`)
            break;
        ...
        case 'V':
            /* Adjust audio volume */
            ui_adjust_volume(menuin);// ADDED BY TERMI (`menuin`)
            break;
