 ...
 */
 PJ_DEF(pj_status_t) pjsua_call_xfer__2( pjsua_call_id call_id, // ADDED BY TERMI
				     const pj_str_t *dest,
				     const pjsua_msg_data *msg_data,
					      ...
					      const pjsua_msg_data *msg_data)
{
    ...
    status = acquire_call("pjsua_call_xfer_replaces()", dest_call_id,
			  &dest_call, &dest_dlg);
    if (status != PJ_SUCCESS) {
	//pj_log_pop_indent();// ADDED BY TERMI: commented by TERMI
	errorCode = 1;// ADDED BY TERMI
	//return status;// ADDED BY TERMI: commented by TERMI
